...
public:
    ...
    bool init(GJGameLevel* level, bool useReplay, bool dontCreateObjects) {
        ...
        speedrun::timer::initializeState(
            ...
            speedrun::utils::parseTargets(
                Mod::get()->getSavedValue<std::string>(speedrun::utils::levelKey(level), "")
            )
        );
        m_fields->m_splitState.m_finalTargetSplit = Mod::get()->getSavedValue<double>(
...
private:
    void reloadStoredSplitTargets() {
        auto storedTargets = speedrun::utils::parseTargets(
            Mod::get()->getSavedValue<std::string>(speedrun::utils::levelKey(this->m_level), "")
        );
        m_fields->m_splitState.m_targetSplits = std::move(storedTargets);
        m_fields->m_splitState.m_finalTargetSplit = Mod::get()->getSavedValue<double>(
            speedrun::utils::finalLevelKey(this->m_level),
            0.0
        );
        m_fields->m_splitState.m_hasFinalTarget = m_fields->m_splitState.m_finalTargetSplit > 0.0;
...
class $modify(MyCheckpointGameObject, CheckpointGameObject) {
    void triggerObject(GJBaseGameLayer* layer, int uniqueID, std::vector<int> const* remapKeys) {
        ...
        static_cast<MyPlayLayer*>(playLayer)->registerCheckpoint(this);
    }
};
